...
def rvfts_forecast(
    ...
    if T < min_T:
        raise ValueError(
            f"Série muito curta: T={T} < ew+cs+h={min_T}. "
            f"Reduza ew/cs ou forneça mais dados."
        )
...
def run_mcs_for_horizon(
    horizon: int,
    results_root: Union[str, Path] = OUTPUT_ROOT,
    *,
    tickers:      Optional[Sequence[str]] = None,
    model_subset: Optional[Sequence[str]] = None,
    verbose: bool = True,
...
def generate_parameters_report(
    ...
    *,
    ...
    """
    if out_path is None:
        out_path = OUTPUT_ROOT / "00_overview" / "rvfts_parameters_report.md"
    out_path.parent.mkdir(parents=True, exist_ok=True)
