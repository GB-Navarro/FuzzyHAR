...
Lê diretamente das pastas que o notebook produz (onde {split} é a
proporção treino/teste — ex.: 40_60, 60_40, 70_30 — definida em SPLIT):

    Others/Predictions/{split}/Fuzzy HAR/RV_t+{h}/{sym}/
        {sym}_tplus{h}_train_with_fuzzy.xlsx   → RV, Target, Fuzzy HAR
        {sym}_tplus{h}_test_with_fuzzy.xlsx

    Others/Predictions/{split}/HAR Models/RV_t+{h}/{sym}/
...
# Proporção de split treino/teste (subpasta dos dados do notebook).
# Opções disponíveis em disco: "40_60", "60_40", "70_30".
SPLIT = "60_40"

# Raízes dos dados produzidos pelo notebook
FUZZY_ROOT      = Path("Others") / "Predictions" / SPLIT / "Fuzzy HAR"
HAR_MODELS_ROOT = Path("Others") / "Predictions" / SPLIT / "HAR Models"

# Pasta de saída desta integração (separada por split para não sobrescrever)
OUTPUT_ROOT = Path("Results") / "HAR_FuzzyHAR_RVFTS_Comparison" / SPLIT
